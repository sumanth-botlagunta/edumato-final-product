    ...
    const listmeal = ({quickdata}) => {
        if (quickdata){
            return quickdata.map((item) =>{
                ...
                <Link to={`Listing/${item.mealtype_id}`} style={{ textDecoration: 'none' }} className="mealtypes" key={item._id}>
                <div className="mealtype" key={item._id}>
                    <div className="mealtype_container" >
                            <div className="meal_thumb">
                                <img src={item.meal_image} alt="mealtype_image"  className="rounded img-fluid"/>
                            </div>
